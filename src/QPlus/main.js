...
_QPlus.getParams = function (id, convert) {
...
_QPlus.versionCheck = function (version, targetVersion) {
...
_QPlus.formatArgs = function (args) {
...
_QPlus.getArg = function (args, regex) {
  var arg = null;
  for (var i = 0; i < args.length; i++) {
...
_QPlus.getMeta = function (string) {
  var meta = {};
  var inlineRegex = /<([^<>:\/]+)(?::?)([^>]*)>/g;
...
_QPlus.getCharacter = function (string) {
...
_QPlus.charaIdToId = function (string) {
  string = String(string).toLowerCase();
  if (/^[0-9]+$/.test(string)) {
...
_QPlus.compareCharaId = function (a, b) {
  if (a === b) return true;
  return this.charaIdToId(a) === this.charaIdToId(b);
};

/**
 * @static _QPlus.request
 ...
 */
_QPlus.request = function (filePath, callback, err) {
 ...
 */
_QPlus.wait = function (duration, callback) {
  var waiter = {
    duration: duration || 0,
    callback: callback,
...
_QPlus.removeWaitListener = function (waiter) {
  var i = this._waitListeners.indexOf(waiter);
  if (i === -1) return;
  this._waitListeners.splice(i, 1);
};

_QPlus.clearWaitListeners = function () {
  this._waitListeners = [];
};

_QPlus.mixin = function (to, what) {
...
_QPlus.mixinWait = function (into) {
  this.mixin(into, {
    wait: this.wait,
 ...
 * @return {Object}
 */
_QPlus.stringToObj = function (string) {
  ...
  lines.forEach(function (value) {
    ...
    if (match) {
      ...
      var arr = _QPlus.stringToAry(match[2].trim());
 ...
 * @return {Array}
 */
_QPlus.stringToAry = function (string) {
...
_QPlus.stringToType = function (string) {
  string = string.trim();
  var rx = _QPlus._regex;
  ...
  if (rx.isArray.test(string)) {
    try {
      return JSON.parse(string).map(_QPlus.stringToType);
    } catch (e) {
      return string;
    }
  }
  if (rx.isObj.test(string)) {
    try {
      ...
      for (var key in obj) {
        obj[key] = _QPlus.stringToType(obj[key]);
      }
      return obj;
    } catch (e) {
      return string;
 ...
 */
_QPlus.pointToIndex = function (point, maxCols, maxRows) {
 ...
 * @return {Point}
 */
_QPlus.indexToPoint = function (index, maxCols, maxRows) {
...
/**
 * @static _QPlus.adjustRadian
 * Keeps the radian between 0 and MAth.PI * 2
 * @param  {Int} radian
 * @return {Int}
 */
_QPlus.adjustRadian = function (radian) {
  while (radian < 0) {
    radian += Math.PI * 2;
  }
...
_QPlus.update = function () {
  this.updateWaiters();
};

_QPlus.updateWaiters = function () {
