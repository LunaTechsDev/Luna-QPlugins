...
(function () {
  var _PARAMS = QPlus.getParams("<Luna_QPathfind>", true);
  ...
  QPathfind.prototype.updateSmart = function () {
    ...
    if (!this._forceReq && this.options.chase !== undefined) {
      ...
      if (!this.options.towards && !this._failed) {
        ...
        if (Imported.QMovement) {
          if (
            this._endNode.x === chasing.px &&
            this._endNode.y === chasing.py
          ) {
            return;
          }
        } else {
  ...
  QPathfind.prototype.aStar = function () {
    ...
    for (i = 0; i < j; i++) {
      ...
      var gScore =
        this._current.g + this.heuristic(this._current, neighbors[i]);
  ...
  if (Imported.QMovement) {
    var Alias_Scene_Map_updateDestination =
